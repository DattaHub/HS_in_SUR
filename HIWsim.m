...
%    Sigmaj(c1,c1)=iwishrnd(DG(cliques(1).ID,cliques(1).ID),bG+cliques(1).dim-1);  % sample variance mx on first component
   Sigmaj(c1,c1)=inv(wishart_InvA_rnd(bG+cliques(1).dim-1,DG(cliques(1).ID,cliques(1).ID),1));  % sample variance mx on first component
...
%        SigRS = iwishrnd(DRS{i},bG+2*(cliques(i).dim-separators(i).dim)-1);
